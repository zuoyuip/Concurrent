...
@Component
@Slf4j
public class VerifyAggregationTasker {

	private static final String RESULT = "result";
	private final ElasticsearchRestTemplate elasticsearchRestTemplate;

	public VerifyAggregationTasker(ElasticsearchRestTemplate elasticsearchRestTemplate) {
	...
	public void verifyAggregation() {
		...
		Query query = new NativeSearchQueryBuilder().withAggregations(resultBuilder).withQuery(rangeQueryBuilder)
				.build();
		SearchHits<VerifyResult> searchHits = elasticsearchRestTemplate.search(query, VerifyResult.class);
		if (searchHits.hasAggregations()) {
			...
			VerifyAggregation verifyAggregation = new VerifyAggregation();
			verifyAggregation.setModule(Modules.XCSZJZGJ);
			verifyAggregation.setDateTime(nowDateTime.getTime());
			verifyAggregation.setCount(countQueryBucketDocCount);

			// 有效率
			verifyAggregation.setValidRate(NumberUtil.div(validQueryBucketDocCount, countQueryBucketDocCount));
			// 成功率
			verifyAggregation.setSuccessRate(NumberUtil.div(successQueryBucketDocCount, countQueryBucketDocCount));
			// 失败率
			verifyAggregation.setFailRate(NumberUtil.div(failQueryBucketDocCount, countQueryBucketDocCount));
			// 超时率
			verifyAggregation.setTimeOutRate(NumberUtil.div(timeOutQueryBucketDocCount, countQueryBucketDocCount));
			elasticsearchRestTemplate.save(verifyAggregation);
		}
